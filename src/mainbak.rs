use binrw::{binrw, BinRead, BinWrite, BinReaderExt, BinWriterExt};
...
fn decrypt_bytes_to_string(encrypted_bytes: &[u8]) -> String {
    ...
    String::from_utf8_lossy(&decrypted_bytes).trim_end_matches('\0').to_string()
}
...
impl IESColumn {
    ...
    pub fn new(column: &str, name: &str, type_data: u16, access_data: u16, sync_data: u16, decl_idx: u16) -> Self {
...
impl IESColumnData {
    pub fn to_serializable(&self) -> IESColumnDataSerializable {
        IESColumnDataSerializable {
            ...
            string_data: self.string_data.iter().map(|s| s.to_serializable()).collect(),
        }
    }
    
    pub fn from_serializable(serializable: &IESColumnDataSerializable, num_column_string: u16) -> Self {
        Self {
            ...
            string_data: serializable.string_data.iter().map(|s| IESRowText::from_serializable(s)).collect(),
...
impl IESHeader {
    pub fn idspace_str(&self) -> String {
        String::from_utf8_lossy(&self.idspace).trim_end_matches('\0').to_string()
    }
    
    pub fn keyspace_str(&self) -> String {
        String::from_utf8_lossy(&self.keyspace).trim_end_matches('\0').to_string()
    }
    ...
    pub fn to_serializable(&self) -> IESHeaderSerializable {
        IESHeaderSerializable {
            ...
            column_data: self.column_data.iter().map(|d| d.to_serializable()).collect(),
        }
    }
    ...
    pub fn from_serializable(serializable: &IESHeaderSerializable) -> Self {
        ...
        Self {
            ...
            padding3: 0,
            columns: serializable.columns.iter().map(|c| IESColumn::from_serializable(c)).collect(),
            column_data: serializable.column_data.iter().map(|d| IESColumnData::from_serializable(d, serializable.num_column_string)).collect(),
        }
    }
...
fn main() {
    ...
    match &cli.command {
        Commands::Read { file } => {
            match IESHeader::read_from_file(file) {
                Ok(header) => {
        ...
        Commands::ExportJson { input, output } => {
            match IESHeader::read_from_file(input) {
                Ok(header) => {
        ...
        Commands::ExportXml { input, output } => {
            match IESHeader::read_from_file(input) {
                Ok(header) => {
        ...
        Commands::ImportJson { input, output } => {
            match IESHeader::from_json(input) {
                Ok(header) => {
        ...
        Commands::ImportXml { input, output } => {
            match IESHeader::from_xml(input) {
                Ok(header) => {
                    if let Err(e) = header.write_to_file(output) {
                        eprintln!("Failed to write IES file: {}", e);
                        process::exit(1);
